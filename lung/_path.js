// V3.9.10（內部審核 P1-7）：共用檔版號 —— 各頁載入後比對，避免「新 HTML + 舊快取 JS」
const PATH_VERSION = 'V3.9.10';
...
function buildPathCore(state){
  ...
  if(t==='SCLC'){
    if(st==='META'){
      ...
      // 依腦/脊髓轉移調整第一線
      if(brain==='no'){
        steps.push({ id:'sclc-es-1l', title:'第一線：化療 + 免疫治療', line:DRUGS.SCLC_ES.line, nhi:'NHI', drugs: DRUGS.SCLC_ES.list, note: DRUGS.SCLC_ES.note });
      } else if(brain==='yes'){
        // V3.9.0（複審 C4）：原本寫「免疫不適用」把「健保不給付」講成「醫學上不能用」。
        //   改由共用規則層回傳，臨床選項與健保給付分開陳述。
        const es = ruleEsSclcImmune('yes');
        steps.push({ id:'sclc-es-1l-brain', title:'第一線：化學治療（合併腦部放射線治療）', line:'一線', nhi:'NHI',
        ...
        // unknown
        steps.push({ id:'sclc-es-1l-unk', title:'第一線：化療為基礎，是否加免疫須先確認腦影像', line:'一線', nhi:'NHI',
    ...
    if(st==='EARLY' || st==='LOCAL'){
      // 侷限型
      const steps = [
        { id:'sclc-ls-ccrt', title:'同步化放療 — 主要治療', line:DRUGS.SCLC_LS_CCRT.line, nhi:'NHI', drugs: DRUGS.SCLC_LS_CCRT.list, note: DRUGS.SCLC_LS_CCRT.note },
  ...
  if(st==='EARLY'){
    ...
    if(!isIA){
      // V3.9.4: 改由規則層依健保 115/6/1 條文判定（原本一句話帶過，且註記「健保尚未給付」已過時）
      const _neo = ruleNeoadjuvant(state, !isNS);
      if(_neo.eligible){
        steps.push({ id:'neoadj-io-chemo', title:_neo.label, line:'術前輔助', nhi:'NHI',
          drugs:_neo.regimen, note:_neo.note });
      } else if(_neo.reason === 'pending'){
        steps.push({ id:'neoadj-pending', title:_neo.label, line:'術前輔助', note:_neo.note });
      }
    }
    // 術後輔助化療：IA 期不需要（觀察為主），II 期或高風險 IB 才推
    if(!isIA){
      steps.push({ id:'postop-adj-chemo', title:'術後輔助化療（II 期或高風險 IB）', line:'術後輔助', nhi:'NHI', drugs:[
    ...
    if(!isIA && isNS){
      steps.push({ id:'postop-addon-any', title:'術後追加治療：依基因檢測結果擇一', line:'術後追加治療（擇一）', nhi:'SELF', drugs:[
  ...
  if(st==='LOCAL'){
    ...
    // V3.9.4: 可切除的 IIIA（N1/N2）也適用術前輔助治療（健保條文含 N1/N2、排除 N3）
    const _neoIII = ruleNeoadjuvant(state, !isNS);
    const steps = [
    ...
    if(isEgfrClassic){
      steps.push({ id:'iii-consol-osi', title:'同步化放療結束後病情穩定者:口服標靶維持治療', line:DRUGS.CONSOLID_OSI.line, nhi:'NHI',
      ...
      steps.push({ id:'iii-consol-durva', title:'同步化放療結束後病情穩定者:免疫維持治療', line:'維持治療', nhi:'NHI',
        drugs: DRUGS.CONSOLIDATION.list, note: DRUGS.CONSOLIDATION.note });
    }
    if(isIIIA && _neoIII.eligible){
      steps.unshift({ id:'neoadj-io-chemo-iiia', title:_neoIII.label + '（可切除者）', line:'術前輔助', nhi:'NHI',
        drugs:_neoIII.regimen, note:_neoIII.note });
    }
    if(isIIIA){
    ...
    if(m==='EGFR_EX20'){
      ...
      steps = [
        { id:'egfr20-1l', title:'第一線：Amivantamab + 化療', line:'一線', nhi:'NHI',
    ...
    } else if(m === 'EGFR_OTHER'){
      // V3.9.3（內部複審 N-08）：EGFR_OTHER 是 uncommon mutation（G719X、S768I、L861Q 等）。
      //   原本在 IV 期與 classic 併為同一分支 → 拿到含 Osimertinib 的「五藥擇一」清單；
      ...
      warns.push('較少見的 EGFR 突變型用藥選擇與常見型不同，建議與主治醫師或多專科團隊討論');
    } else if(EGFR_CLASSIC_SET.includes(m)){   // V3.9.3: EGFR_OTHER 已獨立處理
      ...
      steps = [
        ...
        { id:'egfr-chemo', title:'標靶藥都失效後：接續化學治療', line:'接續治療', nhi:'NHI', drugs:[
            chemoBackbone(isNS)],
      ...
      steps = [
        { id:'alk-1l', title:'第一線：口服 ALK 標靶藥', line:DRUGS.ALK.line, nhi:'NHI', drugs: DRUGS.ALK.list, note: DRUGS.ALK.note },
        { title:'若產生抗藥性：換接續 ALK 標靶藥', line:'接續治療', note:'Lorlatinib 可用於其他 ALK 標靶失敗後。' },
        { id:'alk-chemo', title:'多線標靶失敗後：接續化療', line:'接續治療', nhi:'NHI', drugs:[chemoBackbone(isNS)], note: chemoNote(isNS) },
      ];
    } else if(m==='ROS1'){
      steps = [
      ...
      steps = [
        { id:'braf-1l-chemo', title:'第一線：含鉑類化療', line:'一線', nhi:'NHI', drugs:[chemoBackbone(isNS)], note: chemoNote(isNS) },
        { id:'braf-2l', title:'第二線（化療失敗後）：口服 BRAF 雙標靶', line:DRUGS.BRAF.line, nhi:'NHI', drugs: DRUGS.BRAF.list, note: DRUGS.BRAF.note },
      ];
    } else if(m==='MET'){
      steps = [
      ...
      steps = [
        ...
        { id:'kras-2l', title:'第二線：口服 KRAS G12C 標靶', line:DRUGS.KRAS.line, nhi:'SELF', drugs: DRUGS.KRAS.list, note: DRUGS.KRAS.note },
      ];
      ...
      steps = [
        ...
        { id:'pending-chemo', title:'等報告期間：可先用化療控制', line:'一線', nhi:'NHI', drugs: isNS ? [{n:'Pemetrexed + Carboplatin/Cisplatin',z:'愛寧達 + 鉑類'}] : [{n:'Carboplatin + Paclitaxel',z:'鉑類 + 紫杉醇'}], note:'體力狀況允許時，可先化療爭取時間。' },
      ...
      if(pdl1==='HIGH'){
        ...
        steps = [
          { id:'nodriver-1l', title:'第一線：化療搭配免疫治療', line:DRUGS[k].line, nhi:'NHI', drugs: DRUGS[k].list, note: DRUGS[k].note },
        ...
        steps = [
          ...
          { id:'pending-chemo2', title:'等檢測報告期間：可先用化療', line:'一線', nhi:'NHI', drugs: isNS ? [{n:'Pemetrexed + Carboplatin/Cisplatin',z:'愛寧達 + 鉑類'}] : [{n:'Carboplatin + Paclitaxel',z:'鉑類 + 紫杉醇'}], note:'' },
    ...
    if(state.t790m === 'yes' && EGFR_CLASSIC_SET.includes(m)){
      ...
      if(_alreadyOsi){
        ...
        steps.push({ id:'t790m-2l', title:'已驗出 T790M 抗藥性突變：可換 Osimertinib', line:'二線標靶', nhi:'NHI',
...
function buildPostOpPath(t, st, m, brain, stageIn, state){
  ...
  if(st==='LOCAL'){
    const steps = [surgeryStep];
    steps.push({ id:'postop-adj-chemo-std', title:'術後輔助化療（標準 4 療程）', line:'術後輔助', phase:'adjuvant_chemo', nhi:'NHI',
      drugs:[
        {n:'Cisplatin + Vinorelbine',z:'鉑類 + 溫諾平'},
        ...(isNS ? [{n:'Cisplatin + Pemetrexed（非鱗狀）',z:'鉑類 + 愛寧達'}] : []),
      ],
...
/* V3.9.3（內部複審 N-09/N-10）：原本 fallback 到 phase 或 title。
   `buildRecurrencePath` 會改寫 title（「第一線：」→「復發後接續治療（第一順位）：」），
   key 一變，個管先前的用藥勾選全部失效；phase 又是多個步驟共用的分類不是識別碼。
   現在所有含 drugs 的步驟都有顯式 id；沒有 id 就回空字串（等同不可勾選），
   並在開發環境提示，避免未來新增步驟時再默默退回 title。 */
function stepKeyOf(step){
  if(!step) return '';
  if(step.id) return step.id;
  if(step.drugs && step.drugs.length && typeof console !== 'undefined' && console.warn){
    console.warn('[_path] 步驟含藥物但缺少穩定 id，用藥勾選將無法保存：', step.title);
  }
  return '';
}
...
/* ═══ V3.9.4：術前（前導）輔助治療 — 健保第 9 章 115/6/1、115/8/1 ═══
   條文原文：
     「非小細胞肺癌術前輔助治療：限用於可切除（腫瘤≧4 公分或淋巴結陽性 N1/N2(排除 N3)，
       且無疾病轉移 M0）之**不具 EGFR 或 ALK 腫瘤基因異常**成人病人，且**至多 3 個療程**：
        i. 非鱗狀：限 nivolumab 與 pemetrexed 及含鉑化療併用
       ii. 鱗狀  ：限 nivolumab 與含鉑化療併用」
   ── 系統原本只有一句「術前可先做化療或加免疫」，且註記寫「健保尚未給付，需自費」，
      在 115/6/1 之後是**過時且會誤導病人**的資訊；也完全沒有資格條件判斷。
   ── 「可切除」無法由 TNM 自動判定，一律標示為需由胸腔外科／MDT 判定。          */
function ruleNeoadjuvant(state, isSquamous){
  const mut = (state && state.mut) || '';
  const t = (state && state.t) || '';
  const n = (state && state.n) || '';
  const m = (state && state.m) || '';

  // 排除條件：具 EGFR 或 ALK 基因異常
  const egfrOrAlk = EGFR_ALL_SET.includes(mut) || mut === 'ALK';
  // 尚未檢測 → 無法證明「不具 EGFR/ALK」
  const molUnknown = ['', 'PENDING', 'DECLINED', 'NONE'].includes(mut);
  // M0
  const isM0 = (m === 'M0');
  // 腫瘤 ≧4 公分（T2b 以上多屬之；T3/T4 亦可能因侵犯而非大小 → 標示需確認）
  const sizeGe4 = /^T(2b|3|4)/.test(t);
  // 淋巴結 N1/N2（排除 N3）
  const nodePos = /^N1/.test(n) || /^N2/.test(n);

  const regimen = isSquamous
    ? [{ n:'Nivolumab + 含鉑化療', z:'保疾伏 + 鉑類' }]
    : [{ n:'Nivolumab + Pemetrexed + 含鉑化療', z:'保疾伏 + 愛寧達 + 鉑類' }];
