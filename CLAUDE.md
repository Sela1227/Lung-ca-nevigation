...
### V2.4.0 — 2026-04-06
**肺癌檢查清單與治療選項調整**
- 移除基礎檢查 01 初步評估的「病史及理學檢查（含體能狀況 + 體重減輕）」 — 此項一定會做，不需勾選
- 輔助治療 `getAdjuvantHTML` / `getAdjuvantSummary` 中的「合併化放療」一律改為「術後化放療」（更明確區分輔助 vs 一線治療）
  - 不影響 pathway templates 中針對 unresectable 病灶的「合併化放療（CCRT）」標籤（那是一線治療，不是術後輔助）
- 系統性藥物治療 → 免疫子組新增「免疫 + 化療 + 標靶」三合一選項（id: `io_chemo_target`），對應 IMpower150-style 組合
- 同步更新 KPI 策略檢查（`checkKPI` 的 systemic 分支），加入 `io_chemo_target` 與 `target_chemo` 為合法選項
...
#### V1.2.0 — 2026-04-06（System V2.4.0）
**檢查清單與治療選項調整**
- 基礎檢查移除「病史及理學檢查」（一定會做，不需勾選）
- 輔助治療「合併化放療」更名為「術後化放療」（僅在 `getAdjuvantHTML` / `getAdjuvantSummary` 內，不影響一線 CCRT）
- 系統性藥物 → 免疫組新增「免疫 + 化療 + 標靶」三合一選項
- KPI 檢查同步更新
