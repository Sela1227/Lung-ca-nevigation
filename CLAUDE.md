...
| V3.7.0 | V1.17.0 | 2026-07-04 | **Sela 交辦六項（個管實務回饋）**：(1) edu 抬頭縮小（20→14px，版面讓給分期）(2) **分期加 TNM 明細**（病人常拿這份去第二意見/轉診）(3) **新增「暫不檢測」** — 驅動基因自費，選「沒驗」被反覆催會變推銷；保留「第一步：先完成基因檢測」讓病人知道該做，但移除下方重複催促（僅保留「未驗+PD-L1高」一次安全提醒，因為那是會用錯藥的組合）(4) **科別可標「本次不需要」** → 衛教頁不顯示該科（原本只有「待安排」二元）(5) **藥物勾選**：個管勾選要給病人的藥（預設全選、存紀錄、帶 QR `dx`），主治決定後不必全部上 (6) **EGFR ex19del / L858R 拆開** — **查證健保第 9 章：Bevacizumab+Erlotinib 限「EGFR Exon 21 L858R + 腦轉移 + 非鱗狀 + 第Ⅳ期 + 第一線」**，ex19del 不符合；同時新增 **T790M 疊加勾選**（不與主要突變互斥，健保 9.80 二線 Osimertinib 需 T790M + 曾用一/二代 TKI 失敗）+ NSCLC 腦轉移題（僅「晚期非鱗+L858R」時才問）。252 組 round-trip 全一致 BUG-74 |
