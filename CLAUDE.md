...
| 系統版 | lung 模組 | 日期 | 重點 |
|--------|----------|------|------|
| V3.9.0 | V1.19.0 | 2026-09-14 | **收斂三套平行臨床路徑**（外部「路徑漂移複審」判 P0：`_path.js` 只服務民眾版，醫護版 `PW/PW_SUMMARY/STAGE_GOALS` 與 `edu-pro.html` 各自決策 → V3.6.0 後的臨床修正只進了民眾版，同一病人在不同頁面拿到不同治療順序與藥物）。過渡做法：在 `_path.js` 建**共用臨床規則層**（`ruleConsolidationIII` / `ruleLsSclcConsolidation` / `ruleEsSclcImmune` / `ruleMetaSystemic`），四個頁面全部載入 `_path.js` 並呼叫同一個函式。解掉的分叉：(P0-02) 醫護版把鞏固寫成 CCRT 的**並列選項**（「合併化放療 或標靶 或免疫」）→ 改成正確順序 + driver 分流；edu-pro **無條件** Durvalumab（EGFR 病人也拿到）→ 改依規則層 (P0-03) LS-SCLC 的 Durvalumab 鞏固只存在民眾版 → 醫護主畫面、PW_SUMMARY、edu-pro 全部補上 (P0-04 + C4) ES-SCLC 腦轉移：醫護主卡暗示可加免疫、QR 說不適用 → 統一改成**臨床可用與健保給付分兩行陳述**（原本「免疫不適用」把 reimbursement restriction 講成 medical contraindication）(P1-02) EGFR 後線標題寫「化療搭配免疫」但藥單只有化療 → 依審核建議不臆測補 ICI，先對齊標題、列入待核定 (P1-03) 醫護版 EGFR 一律壓成 `'EGFR'`、位點只放自由文字不參與決策 → 加 `parseEgfrSubtype` 解析成共用 enum。新增 `test/cross_engine.test.js`（53 項 golden-case 一致性斷言）BUG-84 |
| V3.8.5 | V1.18.5 | 2026-09-14 | **外部審核 V3.8.3 報告的四項**：(P1-01) **舊 QR 單一高風險因子被逐字拆開** — `rf` 用「有沒有逗號」判斷新舊格式，但舊格式單項（`LVI`）本來就沒逗號 → 被當短碼串拆成 `LVI,VPI,I`；實測六個舊值全中（`MARGIN`→`MARGIN,A,R,G,I,NX`）。**病人拿舊 QR 會看到病理報告上沒有的高風險因子，影響術後化療討論**。改成先辨識完整舊 enum 再判短碼，並依審核建議在 payload 加 `v:2` schema 版本，解碼端依版本走明確 parser (P1-02) `handleImport` 逐筆 `await put()` → 第 N 筆失敗時前 N-1 筆已寫入卻只顯示「格式錯誤」，使用者再匯入即重複；改成 dry-run + 單一 transaction + 複製後再刪 id (P2-01) README V3.8.1 段被 shell `ps` 輸出污染 50 行（打包時反引號被命令替換）→ 清除並修復表格 (P2-02) **測試隨包附上** `test/qr_payload.test.js`（54 項斷言，`node test/qr_payload.test.js` 可重跑）— 審核指出 P1-01 正是「宣稱舊 QR 全綠但漏網」的例子，測試不隨包外部就無法驗證矩陣涵蓋率。該測試當場就抓到我 V3.8.4 的一處測試盲點 BUG-83 |
| V3.8.4 | V1.18.4 | 2026-09-14 | **同步化放療處方依組織型態分流**（個管覺得「怪怪的」→ 查證屬實）。NCCN 3.2026 NSCL-F 明列：**非鱗狀 preferred 是 Carboplatin/Cisplatin + Pemetrexed**（PROCLAIM 顯示與 cisplatin+etoposide 存活相當但副作用較少），Paclitaxel+Carboplatin 與 Cisplatin+Etoposide 才是兩種組織型態共用；**鱗狀不適用 Pemetrexed**。原本 LOCAL 分支兩種組織型態給同一組（只有後兩種）→ **非鱗狀病人看不到自己的首選處方**。改成依 `isNS` 分流，非鱗狀 4 種（Pemetrexed 組合排前）、鱗狀 2 種，note 也分開說明。另做全面稽核：術後輔助（非鱗多 Cisplatin+Pemetrexed）、META 第一線（非鱗 Pemetrexed / 鱗狀 Paclitaxel）本來就正確，鱗狀路徑無任何 Pemetrexed BUG-82 |
| V3.8.3 | V1.18.3 | 2026-09-14 | **全面修 V3.8.1 短碼造成的顯示破口**（個管回報「主治也變成 ?」→ Sela 要求全面查）。V3.8.1 把固定值改短碼、`tm` 改字串、日期改 YYMMDD，`stateFromPayload`（引擎）有用 qrDec 還原，但**畫面上另外 5 處仍是為長格式寫的**：`d.tm[i]` 取到單一字元（主治變 `0` `:` 等）、`mutDisplay(d.m)` 收到 `e8` 回空（驅動基因整段消失）、`pdl1Display(d.pd)` 同理、SCLC 腦轉移 lookup 用 yes/no/unknown 當 key 收到 `y` → 整行消失、日期顯示 `260914`。**修法不是逐點補，而是在解碼入口加 `normalizeQR(d)` 一次還原成長格式**，下游所有消費端不必改（也自動相容舊 QR）。新增 14 欄位顯示稽核測試 BUG-81 |
| V3.8.2 | V1.18.2 | 2026-09-11 | 修 V3.8.1 的 regression（Sela 實機截圖）：QR 衛教頁分期顯示成「**p?**」。V3.8.1 為了瘦身讓「有完整 TNM 時不送 s/sl/ps」改由 edu 重算，`stateFromPayload` 已改走 `_eduStageInfo`，但**顯示函式 `stageDisplay` 仍直接讀 `d.s`/`d.sl`** → 兩者皆空 → 回傳 '?'。改成同樣走 `_eduStageInfo`；並補 `_eduStageInfo` 回傳 `stage`（exact 唯一解），否則 pT2a N0 M0 會顯示成「約 IB」而非「IB」。**與 BUG-76 同一家族：資料進了引擎沒進 renderer（這次是反過來，引擎改了 renderer 沒跟）** BUG-80 |
| V3.8.1 | V1.18.1 | 2026-09-11 | **QR 瘦身（Sela：圖形太複雜、長輩掃不到；審核 P2-15 也列出膨脹到 856 字元）**。四類優化：(1) **可重算的不送** — 有完整 TNM 時 `s`/`sl`/`ps` 不入 payload，edu 用 `resolveStage(tt,nn,mm)` 重算 (2) **空值不送** — 原本所有欄位都帶 `''` 進 JSON (3) **固定值短碼** — `QR_CODES` 表放 `_path.js` 兩端共用（mut/pdl1/yn/age/ecog/tp/rf/stageCat），解碼查不到就原樣回傳 → **舊 QR 相容** (4) **dx 索引化** — 「步驟鍵\|藥名」× 4 筆（79 bytes）壓成「egfr-1l:1,2,3,4」（~15），edu 用兩段式還原（先以空 drugOff 建步驟取得藥物清單，再把索引翻回藥名）。另：`btoa` 改 **base64url**（`+/=` 在 fragment 被二次編碼會導致 atob 失敗）、**QR canvas 200→300px**（模組密度是掃描成功率的關鍵）。實測最壞情境 **614 → 338 字元，縮減 51%**。順手解 P3-26（edu 呼叫 stateFromPayload 兩次）。96 組 round-trip + 舊 QR 相容 + 四條斷言 + 五態矩陣全綠 BUG-79 |
| V3.8.0 | V1.18.0 | 2026-09-11 | **內部 + 外部兩份審核的上線閘門項目**。內部 P0-1（**我 V3.7.0 造成的 regression**）：拆 EGFR 子型後 `isEgfrClassic = (m==='EGFR'\|\|m==='EGFR_CLASSIC')` 變成死碼 → **第三期 EGFR 病人又拿到 Durvalumab**（V3.6.2 才修好的）。根因是驅動基因清單散在 5 處各寫一份 → 抽 `EGFR_CLASSIC_SET / EGFR_ALL_SET / NON_EGFR_DRIVERS / ALL_DRIVERS / DURVA_EXCLUDED` 共用常數，一次解掉 P0-1 + P1-4（PS3-4 標靶警語漏新 enum）+ P1-5（PD-L1 高安全警語漏 64 組）+ P1-6（drugs 頁配對失效、banner 印出原始 enum 字串）。內部 P0-2：SCLC 紀錄載入時 `recomputeStage()` 把 stageCat 洗成 '' 並連帶清掉 mut/pdl1/brainMet，再按更新即**永久覆寫**→ 加 SCLC 無 TNM 的 early-return + 更新時分期由有變空的確認。內部 P0-3 + 外部 P1-1：`drugOff` 用位置索引 → 步驟插入/三區呈現會讓 key 漂移到別的藥；且「候選集合」被當成「已選定治療」→ 改語意 key（`step.id`）+ 五種狀態矩陣（只選 Osi／只選一代／多候選未定／未縮小／整步取消）全綠。外部 P0-1：還原直接 `put()` 保留備份 id → **靜默覆寫本機病人紀錄** → 改「整庫取代／合併匯入」二選一 + 單一 transaction + dry-run 驗證 + 筆數回報。外部 P0-2：醫護版**完全沒有 escape 工具**，病人欄位與匯入 JSON 直接進 innerHTML、edu-pro 的 QR fragment 亦然 → 加 `esc()` 並套用。外部 P1-2：CSV 未跳脫雙引號且有公式注入 → `csvCell()`。另修 P1-7（查表漏 T1a）、P1-8（Tis+非N0 回空白）、P1-9（need 文字漏 T1c）、P1-10（mutDisplay 守衛漏 DECLINED）、P3-17（版號紀律，title/民眾版都補上 V1.18.0）BUG-78 |
| V3.7.4 | V1.17.4 | 2026-07-04 | Sela 貼 QR 實際畫面，抓到勾選用藥後仍有兩處矛盾：(1) 藥物只剩 Osimertinib，說明卻仍寫「**五藥擇一。Dacomitinib 限無腦轉移。**」— 提到沒顯示的藥。改成依實際勾選改寫（只剩一種 → 「主治醫師已為您選擇 泰格莎…」；剩多種 → 「以上藥物由主治醫師擇一」且只在 Dacomitinib 仍在清單時才提它的限制）(2) **臨床錯誤**：第一線已是 Osimertinib，第 4 步仍列「已驗出 T790M：**可換 Osimertinib**」。但健保 9.80 二線 Osimertinib 限「先前用過 Gefitinib／Erlotinib／Afatinib／Dacomitinib」，**用過 Osimertinib 的不適用** → 會讓病人以為還有藥可換。改成「已驗出 T790M：後續方向需醫師評估」並說明為何不適用。96 組 round-trip + renderSummary 全綠 BUG-77 |
| V3.7.3 | V1.17.3 | 2026-07-04 | Sela 回報兩項：(1) **edu 抬頭與分期區視覺衝突** — `.hdr` 和 `.hero` 是兩個同色綠漸層方塊上下疊著，互相打架。改成 `.hdr` 只是無底色細標題列（下方細線分隔）、`.hero` 是唯一主色塊 (2) **個管勾選用藥後仍出現矛盾敘述** — 第一線已選 Osimertinib，下面仍寫「若第一線用的不是 Osimertinib…可換成 Osimertinib」。加 `drugChoice(state, stepIdx, drugs)` 判斷個管實際選了什麼，三種情境分別給對應敘述（已選 Osimertinib／已縮小但非 Osimertinib／未縮小）；並加通用後處理 `markDroppedSteps`：整步藥物全被取消 → 標 `dropped`，衛教頁整步不顯示。**補修：edu 的 `stateFromPayload` 原本沒把 `dx` 接進 `state.drugOff`，導致 edu 端引擎判斷不到個管選擇**（round-trip 抓到 6 組不一致）。96 組 round-trip + renderSummary 全綠 BUG-76 |
| V3.7.2 | V1.17.2 | 2026-07-04 | 藥名譯名統一（Sela 指正）：**Carboplatin 碳鉑 → 卡鉑**（5 處：drugs-pro、drugs-patient 藥物庫、_path.js 兩處年齡調整警語、patient.html 註解）。Cisplatin 原本就是「順鉑」正確。全檔 grep 確認 0 殘留 |
