...
V3.7.0 Sela 交辦六項（個管實務回饋）：(1) edu 抬頭縮小 (2) 分期加 TNM 明細 (3) 新增「暫不檢測」— 自費項目反覆催會變推銷，保留一次正向說明、移除重複催促（僅留「未驗+PD-L1高」單次安全提醒）(4) 科別可標「本次不需要」衛教頁隱藏 (5) 藥物勾選（預設全選、存紀錄、帶 QR）(6) EGFR ex19del/L858R 拆開 — 查證健保第 9 章：Bevacizumab+Erlotinib 限「L858R + 腦轉移 + 非鱗狀 + 第Ⅳ期 + 第一線」六條件、ex19del 不符；另加 T790M 疊加勾選（9.80 二線需 T790M + 曾用一/二代 TKI 失敗）與 NSCLC 腦轉移題（僅晚期非鱗+L858R 時問）。252 組 round-trip 全一致。BUG-74 教訓：同一個基因陽性≠同一組可用藥（健保次型粒度決定選項粒度）、問診選項要先分清互斥軸與疊加軸、條件式問題按需顯示、自費項目「該提醒」與「該重複提醒」是兩件事、預設值要選「新增項目不會被靜默隱藏」那一邊。下版第一優先：**完整實機驗證 + 審稿方的完整操作情境 regression** — V3.7.0 動了 Q4 結構、QR payload、總覽編輯層，面積大且都在個管日常動線上；第 2 是健保 115/8/1 更新（等 Sela 提供正式 PDF）；第 3 是第二癌別（大腸直腸癌）。
