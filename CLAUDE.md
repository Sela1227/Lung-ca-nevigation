...
| V3.7.1 | V1.17.1 | 2026-07-04 | Sela 回報三項：(0) **修 V3.7.0 的 regression** — 選「暫不檢測」後治療方向整個空白。原因：加藥物勾選時寫 `drugKey(si, d)`，但 `renderTreatmentSteps` 的迴圈索引叫 `i` 不叫 `si` → `renderSummary` 拋 ReferenceError 整段沒渲染（edu 端的 `_si` 同錯）。**`node --check` 抓不到（語法合法、執行才炸），跟 BUG-68 的 DRUGstate 同一類** (1) **病理高風險因子讓個管手動勾選**（LVI／VPI／≥4cm／低分化／切緣陽性／Nx，複選，Q5 病理模式）→ 引擎改寫實：IA 有因子時從「除非病理發現…」改成「您的病理報告顯示：X、Y → 與主治討論是否加術後化療」，IB/II 的 note 也列出實際因子；存紀錄、帶 QR `rf` (2) **緊急回診不要叫病人先聯繫個管師** — 改成「請立即至急診就醫，不需等待門診時間」（原本寫「請立即聯繫個案管理師或回診」，緊急狀況聯繫不上會延誤）。72 組 round-trip 全一致 BUG-75 |
