...
儲存查詢時,若病歷號和姓名都沒填 → 跳提醒,至少填一項才能存。

`saveQuery` 開頭檢查 `S.code` / `S.name`,兩者 trim 後都空就提醒並不存(trim 避免只打空白也算填)。

**為什麼**:民眾版紀錄的兩個用途都靠識別 —— 統計「給哪些病人看過」、個管師「載入某筆修改」。無識別的紀錄統計歸不了人、載入也認不出誰。存檔前強制至少一項,是資料品質的源頭防呆。

**教訓(BUG-58)**:有下游會用到的識別欄位,在「產生資料的那一刻」就該擋,不是等下游各處發現空的才處理。防呆放在 saveQuery 入口比放在統計/載入各出口省事又一致。

### 模組版號

lung **V1.14.3 → V1.14.4**,系統版 V3.4.3 → V3.4.4。
...
### 任務 1:edu-patient QR 衛教頁同步 PD-L1

了結 V3.4.0 留的待辦。V3.4.0 把 patient.html 拆成驅動基因/PD-L1 兩維度,但掃 QR 的衛教頁(edu-patient)沒同步,治療分流會不一致。

**兩處對齊**:
- (送)patient.html `buildEduPayload` 加 `pd` 欄位(QR 帶 pdl1)
- (收)edu-patient `buildPathRawCoreFromData` 讀 `d.pd`,同步成「driver 優先、無 driver 看 pdl1」+ driver+PD-L1高的標靶優先提醒

**舊 QR 相容**:V3.4.2 前印出的 QR 沒有 pd 欄位 → 缺省走「未驗」(NONE→先檢測、driver 仍走標靶),不會壞舊 QR。

驗證:edu 6 組合分流 + 2 個舊 QR 相容情境,全部與 patient.html 一致。

### 任務 2:文字修正

驅動基因 label → 「① 驅動基因　通常擇一,彼此多為互斥」(說明為什麼這組是單選)。

### 兩條教訓(BUG-56)

1. **雙人口資料源改一邊,另一邊是「已知待辦」不是「忘記的 bug」** — V3.4.0 動 patient.html 時就當場把 edu 同步列進下版 #1,V3.4.2 補上。有意識地延後+記錄,比默默漏掉好。
2. **payload 加欄位天然要考慮舊資料相容** — QR 印出去就固定了,新增欄位一定要「缺省時有合理行為」(`d.pd||''`→未驗)。任何「已發出、收不回」的格式(QR/URL/存檔),加欄位都要問「舊的沒這欄位會怎樣」。

### 模組版號

lung **V1.14.1 → V1.14.2**,系統版 V3.4.1 → V3.4.2。至此基因/免疫併存功能(查詢工具 + QR 衛教頁)完整對齊。
...
Sela 回報 V3.4.0 醫護版密碼 4 問題,一次修:

### (1) 輸對密碼進不去

推斷原因:原本用 `<input type="password">`,觸發瀏覽器/密碼管理員自動填,把 value 蓋成記住的其他密碼 → 判斷失敗。改 `type="text"` 明碼 + `.trim()` 比對避開。

### (2) 密碼該在 portal 點醫護版時確認

**整個密碼機制從 lung/index.html 搬到 portal**。原本「進到頁面才擋」,改成「點醫護版當下就擋」:

- portal 點醫護版某癌別卡片 → 彈明碼密碼框 → 輸 cbshow → 對 → 跳轉
- lung/index.html 改輕量守衛:沒經 portal 驗證(直接輸網址)→ 踢回 portal
...
sessionStorage 同 origin 共享:portal 驗證設 `pro_auth` flag,lung 守衛讀得到 → 放行。同 session 再點醫護版免重輸。

### 驗證

node 模擬 4 情境全綠:點醫護版彈框、輸錯擋+錯誤訊息、輸對 cbshow 進入+存 flag、同 session 免重輸。

### 兩條教訓(BUG-55)

1. **純前端密碼別用 `type=password`** — 會招來密碼管理員自動填干擾。「一道門」不是「真帳密」,用 `type=text` 明碼更穩。
2. **「在哪一層擋」要順使用者心智流** — 攔在「做選擇的動作點」(點卡片),不是「到達目的地後」(頁面載入)。

### 安全性說明

純前端密碼 + sessionStorage 守衛擋得住「不小心點進來的人」,擋不住「會看原始碼或改 sessionStorage 的人」。這是「一道門」不是「保險箱」。

### 模組版號

lung **V1.14.0 → V1.14.1**,系統版 V3.4.0 → V3.4.1。

---

## V3.4.0 — 2026-07-02
**醫護版進入密碼 + 民眾版基因/免疫檢測併存**

### 任務 1：醫護版進入密碼

全屏 auth-gate overlay,輸入 **cbshow** 進入。sessionStorage 記住(同 session 免再輸)。

純前端明碼(`AUTH_PASS` 常數),擋非醫護人員 —— **不是高強度安全**(能看原始碼就能繞),但日常使用足夠。

### 任務 2：基因檢測與免疫檢測併存

**問題**:原本 Q4 把驅動基因(EGFR/ALK/ROS1/BRAF/MET/KRAS)跟 PD-L1 擠在同一組單選,互斥。但病人可以同時有 EGFR+ 且 PD-L1 60%,只能選一個。

**這是資料模型錯誤,不是 UI 問題** —— 驅動基因(標靶 biomarker)跟 PD-L1(免疫 biomarker)是正交的兩個維度。

**做法**:Q4 拆兩區
- ① 驅動基因(S.mut,必選):EGFR/ALK/ROS1/BRAF/MET/KRAS/無或未驗
- ② PD-L1 免疫指標(S.pdl1 新欄位,選填):≥50%/<50%/未驗

**治療分流(driver 優先)**:

| 驅動基因 | PD-L1 | 治療方向 |
|---------|-------|---------|
| EGFR/ALK/... | 任意 | 標靶(PD-L1 高也提醒仍標靶優先) |
| 無 | ≥50% | 免疫單藥 |
| 無 | <50% | 化療+免疫 |
| 無 | 未驗 | 先完整檢測 |

**臨床安全把關**:驅動基因陽性 + PD-L1 高時,提醒「PD-L1 高但驅動基因陽性仍標靶優先(免疫對 driver+ 效果差)」—— 避免病人誤以為 PD-L1 高就該免疫。

### 記取 BUG-53 教訓

這次加 S.pdl1 新欄位,**一次盤點所有存取點**(saveQuery/loadRecord/restoreAllUI/restart 四處全同步),沒有再漏。這是 BUG-53「先把 S 所有欄位列一遍」的正面實踐。
...
lung **V1.13.2 → V1.14.0**,系統版 V3.3.2 → V3.4.0。

⚠️ **實機驗證**:密碼框輸 cbshow 進入、Q4 驅動基因+PD-L1 能同時選、EGFR+PD-L1高顯示標靶優先提醒。
⚠️ **待辦**:edu-patient QR 衛教頁的 PD-L1 同步未做(下版 #1)。
