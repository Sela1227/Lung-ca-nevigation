...
### v16 (2026-03-26)
- **QR 碼修正**：payload 加入照護團隊醫師全名（tm 陣列），個管師不再遮罩；只有病人姓名做隱私遮罩
- **edu.html 團隊顯示**：掃碼頁從 QR 讀取醫師名字，有填顯示「○○○ 醫師」，未填顯示「（待安排）」
- **導航 UX 全面優化**：
  - 基礎檢查最後一個 tab：「下一組」→「進入決策 →」直接跳決策頁
  - 進階檢查最後一個 tab：「下一組」→「查看路徑 →」直接跳路徑頁
  - 跨頁按鈕加 pulse 動畫（綠色呼吸光），明確標示這是「離開本頁」
  - 底部全域「下一步/上一步」按鈕顯示目的地名稱（如「基礎檢查 →」「← 病人」）
  - 病人頁底部新增「開始基礎檢查 →」明確入口按鈕

### v17 (2026-03-26)
- 移除路徑頁臨床註記 alert boxes（_N2/_N3/_N4/_N5），減少頁面冗長；保留兩則安全警告（鱗癌 Pemetrexed/Bevacizumab 禁忌、SCLC 早期不可僅觀察）
- 路徑頁加 max-width:960px 居中，不再撐滿全螢幕
